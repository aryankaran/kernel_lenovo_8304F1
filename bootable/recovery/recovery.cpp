...
static const char *DATA_SYSTEM_ROOT = "/data/system"; //add for A2060-136
...
// add HQ_auto_factory resource ---start
static const char *CTS_FLAG = "/cache/ctsflag";
// add HQ_auto_factory resource ---end
...
static void mk_file(void)
{
	FILE *fp;
	fp = fopen_path(CTS_FLAG,"w+");
	if(fp == NULL)
		return;
	fclose(fp);
}
...
static int apply_from_sdcard(Device* device, bool* wipe_cache) {
    ...
    /*Add begin by HQ_lyh 2017-11-07, add for low power reminding*/
    int fd = open ("/sys/class/power_supply/battery/capacity", O_RDONLY);
    if (0 > fd)
    {
        ui->Print("\n Open battery capacity file failed. \n");
        return 0;
    }
    char buffer[10]={0};
    ...
    if (30 > atoi(&buffer[0]))
    {
        ui->Print("\n Low battery power, Please shutdown and charge. \n");
        close(fd);
        return 0;
    }
    close(fd);
    /*Add end by HQ_lyh 2017-11-07, add for low power reminding*/ 
    ui->Print("\n-- Install %s ...\n", path);
    set_sdcard_update_bootloader_message();
...
int main(int argc, char **argv) {
   ...
   //add by dongkai for factoryset
    ...
    //add for A2060-136 start
    ensure_path_mounted(DATA_SYSTEM_ROOT);
	if(remove("/data/system/updateResult.xml") != 0 )
	{
	    LOGE("Error deleting updateResult.xml \n");
	}
	else
	{
        LOGE("updateResult.xml successfully deleted \n");
	}
    //add for A2060-136 end
