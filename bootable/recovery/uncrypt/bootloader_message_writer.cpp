...
#define LOG_TAG "bootloader_message_writer"
#include <log/log.h>
...
static bool write_bootloader_message(const bootloader_message& boot, std::string* err) {
  ...
  ret = mt_write_bootloader_message_mtd(&boot, misc_blk_device.c_str());
  if(ret == MTD_WRITE_FAIL) {
    ALOGE("mt_write_bootloader_message_mtd failed %s\n", misc_blk_device.c_str());
    return false;
  }
  else if(ret == MTD_WRITE_FINISH){
    ALOGE("mt_write_bootloader_message_mtd successfully %s\n", misc_blk_device.c_str());
    return true;
  }
  ...
  if (fsync(fd.get()) == -1) {
    *err = android::base::StringPrintf("failed to fsync %s: %s", misc_blk_device.c_str(),
                                       strerror(errno));
    ///M:ALPS02968971 fsync fail because open with O_SYNC flag
    //return false;
  }
  return true;
}
