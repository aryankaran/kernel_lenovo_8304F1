...
class myThread (multiprocessing.Process):
    def __init__(self,root,file,crc32s,digests):
    ...
    def run(self):
        crc32 = 0
        mymd5=hashlib.md5()
        frb=open(os.path.join(self.root,self.file),'rb')
        while True:
            tmp=frb.read(4*1024)
            if tmp=='':
                break
            crc32 += fileCRC32(os.path.join(self.root,self.file),crc32,tmp)
            mymd5.update(tmp)
        self.crc32s.update({os.path.join(self.root,self.file): crc32})
        self.digests.update({os.path.join(self.root,self.file): mymd5.hexdigest()})
        frb.close()
...
def cal_system_file_crc(file_count,in_dir):
  ...
  fo.write("\n")
  threads = []
  crc32s = multiprocessing.Manager().dict()
  digests = multiprocessing.Manager().dict()

  for root, dirs, files in os.walk(in_dir):
    for file in files:
      if os.path.isfile(os.path.join(root,file)) and not os.path.islink(os.path.join(root,file)):
        thread = myThread(root,file,crc32s,digests)
        thread.start()
  ...
  for root, dirs, files in os.walk(in_dir):
    for file in files:
      ...
      if file=="file_count":
        continue
      if os.path.isfile(os.path.join(root,file)) and not os.path.islink(os.path.join(root,file)):
        crc32=crc32s[os.path.join(root,file)]
      crc32=crc32&0xffffffff
      fo.write(str(number))
      fo.write("\t")
      ...
      if not os.path.islink(os.path.join(root,file)):
        filemd5=encrypt(key,digests[os.path.join(root,file)])
      else:
