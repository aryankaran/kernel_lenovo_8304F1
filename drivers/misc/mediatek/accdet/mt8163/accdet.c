...
/* bianxiaoyun add start */
#define HEADSET_SWITCH_ENABLE  (1)
//#define ACCDET_ADC_CHANNEL      (12)

#if HEADSET_SWITCH_ENABLE 
int gpiopin_hpswitch;
int per_enus_gpio_status=1;
#endif
...
static void accdet_eint_work_callback(struct work_struct *work)
{
	...
	/* bianxiaoyun add start */
#if HEADSET_SWITCH_ENABLE 
	int headset_switch_voltage =0;
	int ret =0;
        //int i=0;
        int tmp_sum_h=0;
        int tmp_sum_l=0;
	int gpiopin_hpswitch_value=0;
#endif
	/* bianxiaoyun add end */
	...
	if (cur_eint_state == EINT_PIN_PLUG_IN) {
		...
		accdet_auxadc_switch(1);
                gpio_direction_output(gpiopin_hpswitch, per_enus_gpio_status);
		msleep(100);	
		gpiopin_hpswitch_value=gpio_get_value(gpiopin_hpswitch);
                //for(i=0;i<20;i++)
                while(1)
                    {
                        msleep(10);
                        headset_switch_voltage	= PMIC_IMM_GetOneChannelValue(8, 1, 1);
                        if(headset_switch_voltage > 250)
                            {
                                tmp_sum_h++;
                                tmp_sum_l=0;
                            }else{
                        ...
                        ACCDET_ERROR("accdet headset_switch_voltage = %d , gpio = %d,tmp_sum_h=%d,tmp_sum_l=%d\n", headset_switch_voltage,gpiopin_hpswitch_value,tmp_sum_h,tmp_sum_l);
                        if((tmp_sum_h==20)||(tmp_sum_l==20))
                                break;
                    }
                accdet_auxadc_switch(0);	
		if(tmp_sum_h==20)
		{
			if (gpiopin_hpswitch_value==1)
				ACCDET_ERROR("headset is omtp\n");
			else
				ACCDET_ERROR("headset is ctia\n");
		}
		else
		{ 
			if (gpiopin_hpswitch_value==1)
			{
				ACCDET_ERROR("headset is ctia\n");
				ret = gpio_direction_output(gpiopin_hpswitch, 0);
				mdelay(100);
			}
			else
			{
				ACCDET_ERROR("headset is omtp\n");
				ret = gpio_direction_output(gpiopin_hpswitch,1);
				mdelay(100);
			}
		}  	
#endif		
...
/*Disable ACCDET*/
		...
		accdet_auxadc_switch(0);
		disable_accdet();
                per_enus_gpio_status=gpio_get_value(gpiopin_hpswitch);
                gpio_direction_output(gpiopin_hpswitch, 1);
                ACCDET_ERROR("gpiopin_hpswitch is reset 1 \n");
		headset_plug_out();
	}
...
static inline int accdet_setup_eint(struct platform_device *accdet_device)
{
	...
	*/
	...
	if (node) {
		...
		/* bianxiaoyun add start */
#if HEADSET_SWITCH_ENABLE 	
		gpiopin_hpswitch = of_get_named_gpio(node, "hpswitch-gpio", 0);
		if (gpiopin_hpswitch < 0)
		{
			ACCDET_ERROR("[Accdet] not find hpswitch-gpio\n");
		}
		ret = gpio_request(gpiopin_hpswitch, "hpswitch-gpio");
		if (ret)
...
#ifdef FOUR_KEY_HEADSET
		if (cali_voltage < 100) {
			ret = IMM_GetOneChannelValue_Cali(g_FourKey_ADC_channel, &ap_fourkey_vol);
			ap_fourkey_vol = ap_fourkey_vol/1000;
			ACCDET_INFO("ap_fourkey_vol = %d mv\n", ap_fourkey_vol);
			if (ret == 0 && ap_fourkey_vol < cali_voltage) {
				cali_voltage = ap_fourkey_vol;
			} else if (ret) {
				ACCDET_ERROR("[Accdet]get 4-key volt fail\n");
				cali_voltage = 1500;
			}
		}
#endif
