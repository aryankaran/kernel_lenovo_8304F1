...
static int KXTJ2_1009_CaliReadSensorData(struct i2c_client *client,struct SENSOR_DATA *pgsensor_data)
{
	...
	GSE_LOG("[xrd]KXTJ2_1009_CaliReadSensorData_START!\n");
    ...
    if((res = KXTJ2_1009_ReadData(client, obj->data))!=0)
    {        
    ...
    {      
           ...
           /* lancelot add for g-sensor cali*/
            ...
            pgsensor_data->x=acc[KXTJ2_1009_AXIS_X]*1000/1024;
            pgsensor_data->y=acc[KXTJ2_1009_AXIS_Y]*1000/1024;
            pgsensor_data->z=acc[KXTJ2_1009_AXIS_Z]*1000/1024;
    }
    GSE_LOG("[xrd]KXTJ2_1009_CaliReadSensorData_END!\n");
    return 0;
}

static int KXTJ2_1009_WriteCalibration_CALI(struct i2c_client *client, int dat[KXTJ2_1009_AXES_NUM])
{

	struct kxtj2_1009_i2c_data *obj = (struct kxtj2_1009_i2c_data*)i2c_get_clientdata(client);
	int err = 0;
	int cali[KXTJ2_1009_AXES_NUM];
	GSE_LOG("[xrd]KXTJ2_1009_WriteCalibration_CALI_START!\n");
	...
	GSE_LOG("[xrd]KXTJ2_1009_WriteCalibration_CALI_END!\n");
	mdelay(1);
	return err;
}
/*----------------------------------------------------------------------------*/
...
{  
	...
	GSE_LOG("[xrd]KXTJ2_1009_ReadCalibrationEx_START!\n");
	#ifdef SW_CALIBRATION
		mul = 0;//only SW Calibration, disable HW Calibration
	#else
		if(err = KXTJ2_1009_ReadOffset(client, obj->offset))
...
static int KXTJ2_1009_WriteCalibration(struct i2c_client *client, int dat[KXTJ2_1009_AXES_NUM])
{
    ...
    unsigned int len;
#endif
GSE_LOG("[xrd]KXTJ2_1009_WriteCalibration_START!\n");
#ifdef SW_CALIBRATION
#else
...
static int KXTJ2_1009_CheckDeviceID(struct i2c_client *client)
{
	...
	if (res <= 0)
	{
		printk("KXTJ2_1009_CheckDeviceID fail\n");
        return KXTJ2_1009_ERR_I2C;
	}
...
/*Kionix Auto-Cali End*/
		...
		/*Begin:add by xurendong, for lenovo gsensor calibration*/	
		gsensor_cali_data.x=acc[KXTJ2_1009_AXIS_X]*1000/1024;
		gsensor_cali_data.y=acc[KXTJ2_1009_AXIS_Y]*1000/1024;
		gsensor_cali_data.z=acc[KXTJ2_1009_AXIS_Z]*1000/1024;
		/*end:add by xurendong, for lenovo gsensor calibration*/
...
static ssize_t store_cali_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	else if(3 == sscanf(buf, "0x%02X 0x%02X 0x%02X", &x, &y, &z))
	{
		...
		if(0 != (err = KXTJ2_1009_WriteCalibration_CALI(client, dat)))
		{
...
static long kxtj2_1009_unlocked_ioctl(struct file *file, unsigned int cmd,unsigned long arg)
{
	...
	switch(cmd)
	{
		...
		case GSENSOR_IOCTL_CALI_READ_AFTER_SENSORDATA:
			...
			KXTJ2_1009_SetPowerMode(obj->client, true);
			KXTJ2_1009_ReadSensorData(client, strbuf, KXTJ2_1009_BUFSIZE);
			sensor_data=gsensor_cali_data;
			if(copy_to_user(data, &sensor_data, sizeof(struct SENSOR_DATA)))
			{
					err = -EFAULT;
					break;	  
			}								 
		...
		case GSENSOR_IOCTL_CALI_READ_SENSORDATA:
			...
			memset(&sensor_data,0,sizeof(struct SENSOR_DATA));
			KXTJ2_1009_SetPowerMode(obj->client, true);		
			KXTJ2_1009_CaliReadSensorData(client, &sensor_data);
		...
		case GSENSOR_IOCTL_SET_CALI:
			#if 0
			GSE_LOG("[xrd][MTK]GSENSOR_IOCTL_SET_CALI!\n");
			data = (void __user*)arg;
			if(data == NULL)
			{
				err = -EINVAL;
		...
		/*Begin:add by xurendong, for LENOVO gsensor calibration*/	
			...
			{
					cali[KXTJ2_1009_AXIS_X] = sensor_data.x * gsensor_gain.x / GRAVITY_EARTH_1000;
					cali[KXTJ2_1009_AXIS_Y] = sensor_data.y * gsensor_gain.y / GRAVITY_EARTH_1000;
					cali[KXTJ2_1009_AXIS_Z] = sensor_data.z * gsensor_gain.z / GRAVITY_EARTH_1000;						  
					err = KXTJ2_1009_WriteCalibration_CALI(client, cali);						 
			}
			break;
		/*End:add by xurendong, for LENOVO gsensor calibration*/

		case GSENSOR_IOCTL_CLR_CALI:
			GSE_LOG("[xrd][MTK]GSENSOR_IOCTL_CLR_CALI!\n");
...
static int kxtj2_1009_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	{
		GSE_ERR("kxtj2_1009_init_client fail\n");
...
if(0 == err )
	{
		GSE_ERR("wangchangzhe:kxtj2_1009_device register successs\n");
	}
	if(0 != err )
	{
	...
	GSE_LOG("%s: OK\n", __func__);    
            g_gsensor_name ="Kionix,kxtj2_1009";//add by zch, for Gsensor Hardwareinfo store
