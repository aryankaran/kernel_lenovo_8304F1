...
*
********************************************************************************/
#define CAM_CAL_DRVNAME "CAM_CAL_DRV_GC5025A"
...
extern int GetAfCode_GC5025A(u16 *Macro,u16 *Inf);
extern int GetAfCode_GC5025Ablx4p(u16 *Macro,u16 *Inf);
...
static long gc5025aotp_Ioctl_Compat(struct file *filp, unsigned int cmd, unsigned long arg)
{
    long ret;
    COMPAT_stCAM_CAL_INFO_STRUCT __user *data32;
    stCAM_CAL_INFO_STRUCT __user *data;
    int err;
    CAM_CALDB("[gc5025a_otp] gc5025a_otp_DEVICE_ID,%p %p %x ioc size %d\n",filp->f_op ,filp->f_op->unlocked_ioctl,cmd,_IOC_SIZE(cmd) );

    if (!filp->f_op || !filp->f_op->unlocked_ioctl)
        return -ENOTTY;
    CAM_CALDB("[gc5025a_otp] gc5025a_otp COMPAT_CAM_CALIOC_G_READ = 0x%lx  CAM_CALIOC_G_READ= 0x%lx \n",COMPAT_CAM_CALIOC_G_READ,CAM_CALIOC_G_READ);
    switch (cmd) 
    {
        case COMPAT_CAM_CALIOC_G_READ:
        {
            ...
            if(err != 0)
                CAM_CALERR("[gc5025a_otp] compat_put_acdk_sensor_getinfo_struct failed\n");
            return ret;
        }
        default:
            {   
                CAM_CALDB("[gc5025a_otp] gc5025aotp_Ioctl_Compat CTL CMD ERROR \n");
                return -ENOIOCTLCMD;
            }
...
static int selective_read_region(u32 sensorID,u32 offset, BYTE* data, u32 size)
{
	...
	else if(sensorID == 0x5025)
	{
       ...
       {
            CAM_CALDB("[gc5025a_otp]selective_read region GetAfCode_GC5025A AFInf= %d, AFMacro=%d \n",GC5025AOTP.AfOtpData.AfInf,GC5025AOTP.AfOtpData.AfMacro);
            memcpy(data, &GC5025AOTP.Data[offset], size);
	   
	   }
    
	}
	else if(sensorID == 0x5026)
    {
       if(GetAfCode_GC5025Ablx4p(&GC5025AOTP.Data[0],&GC5025AOTP.Data[1]))
       {
            CAM_CALDB("[gc5025a_otp]selective_read region GetAfCode_GC5025A AFInf= %d, AFMacro=%d \n",GC5025AOTP.AfOtpData.AfInf,GC5025AOTP.AfOtpData.AfMacro);
            memcpy(data, &GC5025AOTP.Data[offset], size);
       }
...
int gc5025a_read_otp(u16 offset, u8* data, u32 size)
{
...
#endif
{
...
#endif
    CAM_CALERR("[gc5025a_otp]CAM_CAL_Ioctl Enter \n");
    ...
    switch(a_u4Command)
    {
        case CAM_CALIOC_S_WRITE:
            ...
            CAM_CALDB("[gc5025a_otp]Write data %d bytes take %lu us\n",ptempbuf->u4Length, TimeIntervalUS);
#endif
            break;
        case CAM_CALIOC_G_READ:
            ...
            CAM_CALDB("[gc5025a_otp]Read data %d bytes take %lu us\n",ptempbuf->u4Length, TimeIntervalUS);
#endif
...
#ifdef CONFIG_COMPAT
    .compat_ioctl = gc5025aotp_Ioctl_Compat,
#endif
...
inline static int RegisterCAM_CALCharDrv(void)
{
    ...
    {
        CAM_CALERR("[gc5025a_otp]Register device no failed\n");
