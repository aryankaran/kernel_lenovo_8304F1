...
///////////////////////////////////////
static kal_uint8 hi556_read_otp;
...
static int HI556_otp_read(void)
{
	...
	printk("HI556 HI556_otp.info_flag = 0x%x \n", HI556_otp.info_flag);
	if (HI556_otp.info_flag == 0x01)	//Base Info Group1 valid
		...
		LOG_INF("HI556 Info data invalid \n");
	...
	if (info_start_addr != 0)
		...
		for (i = 0; i < 11; i++) 
			...
			LOG_INF("HI556 info_data[%d] = 0x%x \n ", i,info_data[i]);
		}
	}
	...
	{	
            ...
            HI556_otp.module_id = info_data[0];
            HI556_otp.lens_id = info_data[1];
            HI556_otp.module_ver =  info_data[2];
            HI556_otp.software_ver = info_data[3];
            HI556_otp.year = info_data[4];
            HI556_otp.month = info_data[5];
            HI556_otp.day = info_data[6]; 
            HI556_otp.awbstation = info_data[7];
            HI556_otp.pdafstation = info_data[8];
            HI556_otp.afstation = info_data[9];

	}
	else
	{   
            HI556_otp.info_flag = 0;
            LOG_INF("HI556_Sensor: Module information checksum Fail \n ");
	}
	LOG_INF("HI556_module_info checksum = 0x%x ,info_check = 0x%x \n", checksum,info_check);

        //wb read	
	printk("OTP_read_cmos_sensor(0x0423) = 0x%x \n", OTP_read_cmos_sensor(0x0423));
	HI556_otp.wb_flag = OTP_read_cmos_sensor(0x0423);
	...
	if (checksum == wb_check)
	{
            ...
            HI556_otp.RG_ratio_unit = wb_data[0]<<8|wb_data[1];
            HI556_otp.BG_ratio_unit = wb_data[2]<<8|wb_data[3];
            HI556_otp.GbGr_ratio_unit = wb_data[4]<<8|wb_data[5];
            HI556_otp.RG_ratio_golden = wb_data[6]<<8|wb_data[7];
            HI556_otp.BG_ratio_golden = wb_data[8]<<8|wb_data[9];
            HI556_otp.GbGr_ratio_golden = wb_data[10]<<8|wb_data[11];
	}
	else
	{
            HI556_otp.wb_flag = 0;
	...
	LOG_INF("HI556_awb checksum = 0x%x ,wb_check = 0x%x \n", checksum,wb_check);

        //af read	
        printk("OTP_read_cmos_sensor(0x044b) = 0x%x \n", OTP_read_cmos_sensor(0x044b));
        HI556_otp.af_flag = OTP_read_cmos_sensor(0x044b);
        if (HI556_otp.af_flag == 0x01)
                af_start_addr = 0x044c;
        else if (HI556_otp.af_flag == 0x13)
                af_start_addr = 0x0454;
        else if (HI556_otp.af_flag == 0x37)
                af_start_addr = 0x045c;
        else
                LOG_INF("HI556 af data invalid \n");

        LOG_INF("HI556 af_flag = 0x%x \n", HI556_otp.af_flag);
        LOG_INF("HI556 af_start_addr = 0x%x \n", af_start_addr);
	if (af_start_addr != 0) 
        {
	...
	{
            LOG_INF("HI556_Sensor: af checksum PASS\n ");
                   HI556_otp.af_direction = af_data[0];
                   HI556_otp.af_inf = af_data[1]<<8|af_data[2];
                   HI556_otp.af_macro = af_data[3]<<8|af_data[4];
	}
	else
	{
            HI556_otp.af_flag = 0;
            LOG_INF("HI556_Sensor: af checksum Fail\n ");
	}
	LOG_INF("HI556_af checksum = 0x%x ,af_check = 0x%x \n", checksum,af_check);

    LOG_INF("HI556_otp.module_id= 0x%x \n", HI556_otp.module_id);
    LOG_INF("HI556_otp.lens_id= 0x%x \n", HI556_otp.lens_id);
    LOG_INF("HI556_otp.HI556_otp.module_ver= 0x%x \n",HI556_otp.module_ver);
    LOG_INF("HI556_otp.HI556_otp.software_ver= 0x%x \n",HI556_otp.software_ver);
    LOG_INF("HI556_otp.year= 0x%x \n", HI556_otp.year);
    LOG_INF("HI556_otp.month= 0x%x \n", HI556_otp.month);
    LOG_INF("HI556_otp.day= 0x%x \n", HI556_otp.day);
    LOG_INF("HI556_otp.awbstation= 0x%x \n", HI556_otp.awbstation);
    LOG_INF("HI556_otp.pdafstation= 0x%x \n", HI556_otp.pdafstation);
    LOG_INF("HI556_otp.afstation= 0x%x \n", HI556_otp.afstation);

    LOG_INF("HI556_otp.RG_ratio_unit= 0x%x \n", HI556_otp.RG_ratio_unit);
    LOG_INF("HI556_otp.BG_ratio_unit= 0x%x \n", HI556_otp.BG_ratio_unit);
    LOG_INF("HI556_otp.GbGr_ratio_unit= 0x%x \n", HI556_otp.GbGr_ratio_unit);
    LOG_INF("HI556_otp.RG_ratio_golden= 0x%x \n", HI556_otp.RG_ratio_golden);
    LOG_INF("HI556_otp.BG_ratio_golden= 0x%x \n", HI556_otp.BG_ratio_golden);
    LOG_INF("HI556_otp.GbGr_ratio_golden= 0x%x \n", HI556_otp.GbGr_ratio_golden);
    LOG_INF("HI556_otp.af_direction= 0x%x \n", HI556_otp.af_direction);
    LOG_INF("HI556_otp.af_inf= 0x%x \n", HI556_otp.af_inf);
    LOG_INF("HI556_otp.af_macro= 0x%x \n", HI556_otp.af_macro);
    ...
    write_cmos_sensor_8(0x0a00, 0x01); //sleep Off
        ...
        return HI556_otp.info_flag;
}
static int HI556_otp_apply_wb(void)
{
	int R_gain = 1, G_gain = 1, B_gain = 1;
	int RG_ratio_golden = HI556_otp.RG_ratio_golden;
	int BG_ratio_golden = HI556_otp.BG_ratio_golden;
        if (!RG_ratio_golden||!BG_ratio_golden) 
        {
           LOG_INF("HI556 HI556_otp_apply_wb No AWB Golden data\n");
           RG_ratio_golden = 0x1fd; //module house provide
           BG_ratio_golden = 0x24e; //module house provide
        }

	LOG_INF("HI556 HI556_otp_apply_wb \n");
	
        if (HI556_otp.wb_flag) 
        {

        printk("HI556_otp_apply-778-RG_ratio_golden=0x%x,BG_ratio_golden=0x%x\n", RG_ratio_golden, BG_ratio_golden);
        R_gain = (0x100 * RG_ratio_golden / HI556_otp.RG_ratio_unit);
	B_gain = (0x100 * BG_ratio_golden / HI556_otp.BG_ratio_unit);
	G_gain = 0x100;
	printk("HI556_otp_apply-782-R_gain=%d,B_gain=%d\n",R_gain,B_gain);
	...
	if (R_gain < B_gain) 
		...
		if(R_gain < 0x100) 
			...
			R_gain = 0x100;
                        printk("HI556_otp_apply-789-R_gain=%d,B_gain=%d\n",R_gain,B_gain);
		}
	} 
        else 
        {
		if (B_gain < 0x100) 
			...
			B_gain = 0x100;
			printk("HI556_otp_apply-796-R_gain=%d,B_gain=%d\n",R_gain,B_gain);
		}
	}
	//R_gain = 0x11111111;
	...
	(read_cmos_sensor(0x0078) << 8) | (read_cmos_sensor(0x0079) & 0xFFFF), (read_cmos_sensor(0x007c) << 8) | (read_cmos_sensor(0x007d) & 0xFFFF), (read_cmos_sensor(0x007e) << 8) | (read_cmos_sensor(0x007f) & 0xFFFF));
        ...
        {
            printk("HI556_otp_apply-wb wb_flag is invalid!\n");
        }
	return HI556_otp.wb_flag;
}
int GetAfCode_HI556(u16 *Inf,u16 *Macro)
{
    if (HI556_otp.af_flag)
    {   
        LOG_INF("GetAfCode_HI556 inf= %d macro=%d\n",HI556_otp.af_inf,HI556_otp.af_macro);
        *Macro = HI556_otp.af_macro;
	*Inf   = HI556_otp.af_inf;
    }
    else
    {    
        LOG_INF("Hi556 Module no af calibration data ! use default.\n");
        *Macro  = 400;
        *Inf    = 200;
    }
	return 1;
...
{    
	kal_uint8 i = 0;
	kal_uint8 retry = 2;
	LOG_INF("hi556qun3p:[get_imgsensor_id]  ");
			...
			do {
				*sensor_id =return_sensor_id();
                if (*sensor_id == HI556_SENSOR_ID)
                {
                		if (hi556_read_otp != 1) 
                        {
                        	LOG_INF("hi556qun3p:get_imgsensor_id() read otp first time\n");
                            sensor_init();
                            HI556_OTPSetting();
                            HI556_otp_read();
                        }
                        ...
                        {
                        	LOG_INF("hi556qun3p:get_imgsensor_id() otp already read \n");
                        }
                        if (HI556_otp.info_flag && HI556_otp.lens_id == 0x01)      
                        {

                             LOG_INF("hi556qun3p:get_imgsensor_id() Module QUNHUI3P \n");
                        }
                        else if (HI556_otp.info_flag && HI556_otp.lens_id == 0x02)
                        {
                                LOG_INF("hi556qun3p:get_imgsensor_id() Module QUNHUI4P \n");
                                *sensor_id+=1;
                        }
                        else
                        {
                                LOG_INF("hi556qun3p:get_imgsensor_id() not a QUNHUI  Module \n");
                        }
				}
                if (*sensor_id == imgsensor_info.sensor_id) {
					LOG_INF("hi556qun3p:get_imgsensor_id() i2c write id  : 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);	  
					return ERROR_NONE;
				}	
				LOG_INF("hi556qun3p:get_imgsensor_id() get_imgsensor_id Read sensor id fail, i2c write id: 0x%x,sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
				retry--;
			} while(retry > 0);
			i++;
			retry = 2;
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
            ...
            if (sensor_id == HI556_SENSOR_ID) 
            {
                if (hi556_read_otp != 1) 
                {
                    LOG_INF("hi556qun3p:open() read otp first time\n");
                    sensor_init();
                    HI556_OTPSetting();
                    HI556_otp_read();
                }
                ...
                {
                    LOG_INF("hi556qun3p:open() otp already read \n");
                }
                if (HI556_otp.info_flag && HI556_otp.lens_id == 0x01)
                {

                        LOG_INF("hi556qun3p:open()  Module QUNHUI3P \n");
                }
                else if(HI556_otp.info_flag && HI556_otp.lens_id == 0x02)
                {
                        LOG_INF("hi556qun3p:open()  Module QUNHUI4P \n");
                        sensor_id += 1;
                }
                else
                {
                        LOG_INF("hi556qun3p:open() not a QUNHUI  Module \n");
                }
			}
			if (sensor_id == imgsensor_info.sensor_id) {				
				LOG_INF("hi556qun3p:open() i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);	  
				break;
			}	
			LOG_INF("hi556qun3p:open() Read sensor id fail open i2c write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		if (sensor_id == imgsensor_info.sensor_id)
