...
//A2060-358,add by guoliang for sd card esd recovery
...
{
	/*Begin:A2060-358,modify by guoliang for sd card esd recovery*/
	int ret;
	ret = mmc_send_status(host->card, NULL);
	if(ret)
		ret = mmc_sd_reset(host);
	return ret;
	/*End:A2060-358,modify by guoliang for sd card esd recovery*/
}
