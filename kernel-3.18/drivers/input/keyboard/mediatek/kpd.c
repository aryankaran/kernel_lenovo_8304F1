...
/*Begin:add by guoliang,for press power key hold 3s shutdown in meta mode.2017-11-02*/
...
#endif
/*End:add by guoliang,for press power key hold 3s shutdown in meta mode.2017-11-02*/
...
void kpd_pwrkey_pmic_handler(unsigned long pressed)
{
    ...
    if(get_boot_mode() == META_BOOT)
    {
        if(0x01 == pressed)
        {
            ...
            hrtimer_start(&meta_poweroff_timer,ktime_set(META_PWRKEY_LONG_PRESS_POWEROFF_TIME,0),HRTIMER_MODE_REL);
            pwrap_read((u32)RTC_SPAR0,&spar0_read);
            spar0 = (u16)spar0_read;
            pwrap_write(RTC_SPAR0,spar0 | (0x01 << 10));
        }
    }
    else if(0x0 == pressed)
    {
        if(meta_poweroff_timer_started)
        {
            ...
            kpd_print("try to cancel meta_poweroff_timer\n");
            meta_poweroff_timer_started = false;
            kpd_print("meta_poweroff_timer canceled\n");
        }
    }
    #endif
    /*End:add by guoliang,for press power key hold 3s shutdown in meta mode.2017-11-02*/
