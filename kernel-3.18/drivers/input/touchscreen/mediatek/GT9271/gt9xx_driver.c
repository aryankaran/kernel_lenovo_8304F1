...
#define CONFIG_GTP_USE_PINCTRL1 1
 ...
 * Distinguish by macro CONFIG_GTP_USE_PINCTRL1
 */
...
s32 gtp_init_panel(struct i2c_client *client)
{
	...
	(!cfg_info_len[3]) && (!cfg_info_len[4]) && (!cfg_info_len[5])) {
		...
		GTP_INFO("Sensor_ID: %d", sensor_id);
                ...
                if (0==sensor_id)
                    tpd->hq_ctp_module_name = "GT9271-YJ";
                else if(2 == sensor_id )
                    tpd->hq_ctp_module_name = "GT9271-DJ";
...
#else
	{
		...
		if (ret == SUCCESS) {
			...
			ctp_config_version = drv_cfg_version;
	...
	/*Begin:add by zch, for CTP Hardwareinfo store*/
	GTP_ERROR(" GT9XX_read_reg version : %d config_version£º%d\n",tpd_info.vid,ctp_config_version);
	tpd->hq_ctp_firmware_version  = tpd_info.vid;
	tpd->hq_ctp_config_version  = ctp_config_version;
	/*End:add by zch, for CTP Hardwareinfo store*/
...
#endif
GTP_ERROR("zch-- in tpd_i2c_probe\n");
