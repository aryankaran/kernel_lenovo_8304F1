...
//static struct i2c_board_info __initdata i2c_tpd = { I2C_BOARD_INFO(GSL_DEV_NAME, (0x80>>1))};
...
static void read_firmware_version(struct i2c_client *client)
{
	u8 read_buf[4]  = {0};
	u8 write_buf[4];
	write_buf[0]=0x3;write_buf[1]=0;write_buf[2]=0;write_buf[3]=0;
	msleep(30);
	//i2c_smbus_read_i2c_block_data(client,0xb0, sizeof(read_buf), read_buf);
	gsl_read_interface(client, 0xb0, read_buf, sizeof(read_buf));
	if (read_buf[3] != 0x5a || read_buf[2] != 0x5a || read_buf[1] != 0x5a || read_buf[0] != 0x5a)
	{
		printk("gsl-----------#########check mem read 0xb0 = %x %x %x %x #########\n", read_buf[3], read_buf[2], read_buf[1], read_buf[0]);
		//init_chip(client);
	}
	//add read firmware_version for HQ
	//i2c_smbus_write_i2c_block_data(client, 0xf0, 4 , write_buf); 
	gsl_write_interface(client, 0xf0, write_buf, 4);
	msleep(10);
	//i2c_smbus_read_i2c_block_data(client, 0x04, 4 , read_buf);
	gsl_read_interface(client, 0x04, read_buf, 4);
	printk("xrd-----------#########check read_firmware_version 0x04  01= %x %x %x %x #########\n", read_buf[3], read_buf[2], read_buf[1], read_buf[0]);
	msleep(10);
	//i2c_smbus_write_i2c_block_data(client, 0xf0, 4 , write_buf); 
	gsl_write_interface(client, 0xf0, write_buf, 4);
	msleep(10);
	//i2c_smbus_read_i2c_block_data(client, 0x04, 4 , read_buf);
	gsl_read_interface(client, 0x04, read_buf, 4);
	printk("xrd-----------#########check read_firmware_version 0x04  02= %x %x %x %x #########\n", read_buf[3], read_buf[2], read_buf[1], read_buf[0]);
	tpd->hq_ctp_firmware_version  =  ((read_buf[3]<<24)|(read_buf[2]<<16)|(read_buf[1]<<8)|(read_buf[0]));;
	tpd->hq_ctp_config_version =  ((read_buf[3]<<24)|(read_buf[2]<<16)|(read_buf[1]<<8)|(read_buf[0]));;
	printk("tpd->hq_ctp_firmware_version = %04x\n",tpd->hq_ctp_firmware_version);
	printk("tpd->hq_ctp_config_version = %04x\n",tpd->hq_ctp_config_version);
}
#ifdef TPD_PROC_DEBUG
...
//	node = of_find_matching_node(node, touch_of_match);
	...
	if (node) {
		...
		ret = request_irq(touch_irq, (irq_handler_t)tpd_eint_interrupt_handler,IRQF_TRIGGER_RISING, "TOUCH_PANEL-eint", NULL);
			if (ret > 0)
				printk("tpd request_irq IRQ LINE NOT AVAILABLE!.");
	} else {
		printk("[%s] tpd request_irq can not find touch eint device node!.", __func__);
	}
			printk("[%s]xxx gsl------------------------tpd request_irq can not find touch eint device node!.", __func__);
...
//	s32 ret = 0;
	...
	read_firmware_version(ddata->client);

	tpd->hq_ctp_module_name = "GSL3680-YJ";
...
static void gsl_suspend(struct device *h)
{
	...
	disable_irq(touch_irq);
	tpd_gpio_output(GTP_RST_PORT, 0);
	printk("gsl_suspend end\n");
}

static void gsl_resume(struct device *h)
{
	printk("gsl_resume start\n");
#ifdef TPD_PROXIMITY
