...
#if defined(EFUSE_CALI)
#if defined(ADC_CALI_EXT_CHANNEL)
...
#if defined(ADC_CALI_EXT_CHANNEL)
	if (0 != get_devinfo_with_index(EfUSE_INDEX))
		mt_auxadc_get_cali_data(channel[dwChannel], data, true);
	else
		mt_auxadc_get_cali_data(channel[dwChannel], data, false);
#else
	mt_auxadc_get_cali_data(channel[dwChannel], data, true);
#endif
...
#if defined(ADC_CALI_EXT_CHANNEL)
			pr_debug("[adc_driver]: channel[%d]=%d.%03d\n", i, data[0], data[2]);
			sprintf(buf_temp, "channel[%d]=%d.%03d\n", i, data[0], data[2]);
#else
			pr_debug("[adc_driver]: channel[%d]=%d.%02d\n", i, data[0], data[1]);
			sprintf(buf_temp, "channel[%d]=%d.%02d\n", i, data[0], data[1]);
#endif
...
static int mt_auxadc_probe(struct platform_device *dev)
{
	...
	/*Begin:A2060-317,add by guoliang after MTK checked for userdebug build reset*/
	mt_auxadc_power_on();
	mdelay(1);
	/*End:A2060-317,add by guoliang after MTK checked for userdebug build reset*/
