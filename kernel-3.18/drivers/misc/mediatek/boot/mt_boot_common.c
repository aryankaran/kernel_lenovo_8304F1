...
enum boot_mode_t g_boot_mode_ex __nosavedata = UNKNOWN_BOOT;//add by zch , for FTM mode
...
static int __init dt_get_boot_common(unsigned long node, const char *uname, int depth, void *data)
{
	...
	if (tags) {
		...
		/*zch add   begin*/
		if (FACTORY_TEST_APK_BOOT == g_boot_mode)
		{
			g_boot_mode = NORMAL_BOOT;
			g_boot_mode_ex = FACTORY_TEST_APK_BOOT;
		}
		/*zch add   end*/
	...
	printk("Gino %s g_boot_mode=%d, g_boot_mode_ex=%d\n", __func__, g_boot_mode, g_boot_mode_ex);
...
void init_boot_common(unsigned int line)
{
	...
	else
	{
            ...
            /*zch add   begin*/
            printk("Gino g_boot_mode=%d\n",g_boot_mode);
            if(g_boot_mode == FACTORY_TEST_APK_BOOT)// FACTORY_TEST_APK_BOOT =11
            {
               g_boot_mode_ex = FACTORY_TEST_APK_BOOT;
               g_boot_mode=NORMAL_BOOT;
            }
...
/*add by zch , for FTM mode,  call in hq_hardwareinfo.c [begin]*/
enum boot_mode_t get_boot_mode_ex(void)
{
    return g_boot_mode_ex;
}
EXPORT_SYMBOL(get_boot_mode_ex);
/*add by zch , for FTM mode,   call in hq_hardwareinfo.c [end]*/
...
static ssize_t hq_apk_debug_read( struct file *file, char __user *buffer, size_t count, loff_t *ppos)
{
	char *page = NULL;
	char *ptr = NULL;
	int len, err = -1;
	...
	ptr = page;
	if (FACTORY_TEST_APK_BOOT == g_boot_mode_ex)
	{
		buf[0] = 1;
	}
	else
...
static int __init boot_common_core(void)
{
	...
	/*zch add   begin*/
	/* create proc entry at /proc/boot_status  for HQ Factory APK Test mode*/
	if (proc_create_data("boot_status", S_IRUGO, NULL, &boot_status_proc_fops, NULL) == NULL)
		pr_warn("create boot_status procfs fail");
	/*zch add   end*/
