...
extern int get_backcam_id_gpio_status(void);
...
//modify by zch for ata crc fail
//    .checksum_value = 0xf7375923,        //checksum value for Camera Auto Test
    .checksum_value = 0x4c23ea4,        //checksum value for Camera Auto Test
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0xfe,0x00);
	write_cmos_sensor(0xfe,0x00);
	write_cmos_sensor(0xf7,0x03);//0x01
	write_cmos_sensor(0xf8,0x1b);//0c
	write_cmos_sensor(0xf9,0x42);
	write_cmos_sensor(0xfa,0x88);
	write_cmos_sensor(0xfc,0x8e);
	write_cmos_sensor(0xfe,0x00);
	...
	write_cmos_sensor(0x02,0x33); //0x33
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    kal_uint8 retry = 2;
	//int backcamid = 0;
	//int gpio_backid_status = 0;
    LOG_INF("gc2375:get_imgsensor_id() ++++"); 
...
// modified for camid by HQ_maoqiming 2017-04-13
			/*gpio_backid_status = get_backcam_id_gpio_status( );
			LOG_INF("[++++gc2375_cxt_gpio-id++++]vendor = %s\n", (gpio_backid_status == 1)?"bolixin":"chengxt");
			if (gpio_backid_status == 1) 
			{
				backcamid=1;
				*sensor_id += 1; 
			} 
		
			LOG_INF("[++++gc2375_cxt_gpio-id++++]*sensor_id=%x,backcamid = %d,gpio_backid_status=%d\n",*sensor_id,backcamid,gpio_backid_status);
	    */if (*sensor_id == imgsensor_info.sensor_id /*&& backcamid ==0*/) {
                LOG_INF("read CXT camera success i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
            }
