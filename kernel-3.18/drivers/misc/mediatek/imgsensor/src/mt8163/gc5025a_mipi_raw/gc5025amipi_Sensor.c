...
//static kal_uint32 Basic_Dummy_Line = GC5025A_DEFAULT_DUMMY_LINE_NUMS;
...
static imgsensor_info_struct imgsensor_info = {
       ...
       .checksum_value = 0xdcdfcdbb, // modify by zch for auto test
...
static kal_uint8 gc5025a_otp_read;
...
static void gc5025a_gcore_read_otp_info(void)
{
...
#ifdef GC5025AOTP_FOR_CUSTOMER
	kal_uint8 infowb_flag,af_flag;
	kal_uint8 info[14];
        kal_uint8 afcode[4];
        kal_uint8 InfHigh,MacHigh;
	//kal_uint8 wb[5];
	//kal_uint8 golden[5];
#endif	
	...
	case 0x01:	
		...
		{
			LOG_INF("GC5025A_OTP_DD : DD check sum correct! checksum = 0x%x\n", ddchecksum);
		}
		else
		{
		...
		{
			LOG_INF("GC5025A_OTP_DD:index = %d, data[0] = %x , data[1] = %x , data[2] = %x ,data[3] = %x \n",\
			...
			if (ddtempbuff[4 * i + 3] & 0x10)
			{
				...
				case 3:
					for (j = 0; j < 4; j++)
					{
						gc5025a_otp_info.dd_param_x[cnt] = (((kal_uint16)ddtempbuff[4 * i + 1] & 0x0f) << 8) + ddtempbuff[4 * i];
						gc5025a_otp_info.dd_param_y[cnt] = ((kal_uint16)ddtempbuff[4 * i + 2] << 4) + ((ddtempbuff[4 * i + 1] & 0xf0) >> 4) + j;
				...
				default:
					gc5025a_otp_info.dd_param_x[cnt] = (((kal_uint16)ddtempbuff[4 * i + 1] & 0x0f) << 8) + ddtempbuff[4 * i];
					gc5025a_otp_info.dd_param_y[cnt] = ((kal_uint16)ddtempbuff[4 * i + 2] << 4) + ((ddtempbuff[4 * i + 1] & 0xf0) >> 4);
...
#ifdef GC5025AOTP_FOR_CUSTOMER	
	gc5025a_select_page_otp(otp_page1);
	infowb_flag = gc5025a_read_otp(0x00);//read InfoWBFlag
	af_flag =     gc5025a_read_otp(0x1d);//read AFFlag
	LOG_INF("GC5025A_OTP : infowb_flag = 0x%x , af_flag = 0x%x\n",infowb_flag,af_flag);
//INFO&WB
        if(infowb_flag == 0x01) 
        {   
            gc5025a_otp_info.infowbvalid = 1;
            gc5025a_read_otp_group(1, &info[0], 14);       
        }
        else if(infowb_flag == 0x07)
        {
            gc5025a_otp_info.infowbvalid = 2;
            gc5025a_read_otp_group(15, &info[0], 14);
        }
        else
        {   
            gc5025a_otp_info.infowbvalid = 0;
            LOG_INF("infowb_flag  invalid !!\n");
        }
        ...
        {   
            ...
            for (i=0;i<13;i++)
            {
                LOG_INF("infowb  Group 1 value =%x !!\n",info[i]);
                check += info[i];
            }
            if ((check % 255 + 1) == info[13])
            {
                LOG_INF("AWB checksum is ok !!\n");
            ...
            {   gc5025a_otp_info.infowbvalid = 0;
                LOG_INF("GC5025A_OTP_INFO Check sum %d Error !!\n", info[13]);
            }
        ...
        {   
            ...
            gc5025a_read_otp_group(30, &afcode[0], 4);       
        }
        else if(af_flag == 0x07)
        {   
            gc5025a_otp_info.afvalid = 2;
            gc5025a_read_otp_group(34, &afcode[0], 4);
        }
        else
        {   
            gc5025a_otp_info.afvalid = 0;
            LOG_INF("af_flag  invalid !!\n");
        }
        if (gc5025a_otp_info.afvalid) 
        {
            ...
            for (i=0;i<3;i++)
            {
                LOG_INF("af Group 1 value =%x !!\n",afcode[i]);
                check += afcode[i];
            }
            if ((check % 255 + 1) == afcode[3])
            {   
                LOG_INF("AF checksum is ok !!\n");
                InfHigh=  afcode[0]>>4;
            ...
            {
                gc5025a_otp_info.afvalid=0;
                LOG_INF("GC5025A_OTP_AF Check sum %d Error !!\n", info[3]);
            }
	...
	LOG_INF("GC5025A_OTP_GOLDEN:golden_bg=0x%x\n",gc5025a_otp_info.golden_bg);
        LOG_INF("GC5025A_OTP_GOLDEN:af_inf=0x%x\n",gc5025a_otp_info.af_inf);
	LOG_INF("GC5025A_OTP_GOLDEN:af_macro=0x%x\n",gc5025a_otp_info.af_macro);	
#endif	
}
...
static void gc5025a_gcore_update_chipversion(void)
{
	...
	for(i=0;i<gc5025a_otp_info.reg_num;i++) 
	{
		...
		LOG_INF("GC5025A_OTP_UPDATE_CHIP_VERSION:{0x%x,0x%x}!!\n",gc5025a_otp_info.reg_addr[i],gc5025a_otp_info.reg_value[i]);		
	}
...
int GetAfCode_GC5025A(u16 *Inf,u16 *Macro)
{
    if (gc5025a_otp_info.afvalid)
    {
        *Macro = gc5025a_otp_info.af_macro;
	*Inf   = gc5025a_otp_info.af_inf;
    }
    else
    {
        *Macro  = 400;
...
static void set_dummy(void)
{
	...
	//hb = imgsensor.dummy_pixel + GC5025A_DEFAULT_DUMMY_PIXEL_NUMS;
...
{	
	...
	else
	{
		...
		write_cmos_sensor(0xb1, temp>>6);
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);
		LOG_INF("GC5025AMIPI analogic gain 2.8x, GC5025AMIPI add pregain = %d\n", temp);
	}
	return gain;
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x1c, 0x2c);//1c 2017.08.08
    write_cmos_sensor(0x1d, 0x00); 
	write_cmos_sensor(0x1e, 0xa0);//90
	write_cmos_sensor(0x1f, 0xb0);
	write_cmos_sensor(0x20, 0x22);//2b
	write_cmos_sensor(0x21, 0x22);//2b
	write_cmos_sensor(0x26, 0x22);//2b
	...
	write_cmos_sensor(0x2f, 0x4d);//4a
	write_cmos_sensor(0x30, 0x11);
	write_cmos_sensor(0x31, 0x20);
	write_cmos_sensor(0x32, 0xc0);//a0 20170919
	write_cmos_sensor(0x33, 0x00);
	write_cmos_sensor(0x34, 0x55);//52//55
	write_cmos_sensor(0x38, 0x04);//02
	write_cmos_sensor(0x39, 0x02);//00
	write_cmos_sensor(0x3a, 0x00);
	write_cmos_sensor(0x3b, 0x00);
	write_cmos_sensor(0x3c, 0x08);//02
	write_cmos_sensor(0x3d, 0x0f);//02
	write_cmos_sensor(0x81, 0x50);//60
	write_cmos_sensor(0xcb, 0x02);
	write_cmos_sensor(0xcd, 0x4d);//2d//ad
	write_cmos_sensor(0xcf, 0x50);
	write_cmos_sensor(0xd0, 0xb3);
	write_cmos_sensor(0xd1, 0x19);//18
	write_cmos_sensor(0xd3, 0xc4);//cc 20170919
	write_cmos_sensor(0xd9, 0xaa);
	write_cmos_sensor(0xdc, 0x03);
	write_cmos_sensor(0xdd, 0xf0);//aa
	write_cmos_sensor(0xe0, 0x00);
	write_cmos_sensor(0xe1, 0x0a);//06 20161205
	write_cmos_sensor(0xe3, 0x2a);
	write_cmos_sensor(0xe4, 0xc0);//a0 20170919
	write_cmos_sensor(0xe5, 0x06);
	write_cmos_sensor(0xe6, 0x10);
	write_cmos_sensor(0xe7, 0xc3);//c2
	...
	write_cmos_sensor(0xfe, 0x01);
	write_cmos_sensor(0x88, 0x00);//f7 20170919
	...
	write_cmos_sensor(0x43, 0x08);/* add offset*///03 20170919
	write_cmos_sensor(0xae, 0x40);/* WB offset*/
	write_cmos_sensor(0x60, 0x00);
	write_cmos_sensor(0x61, 0x80);

	/*gain*/
	write_cmos_sensor(0xb0, 0x4b);//58
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    kal_uint8 i = 0;
    kal_uint8 retry = 2;
    LOG_INF("GC5025ABLX3P:get_imgsensor_id +++++\n");
    	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        	...
        	do {
            	*sensor_id = return_sensor_id();
                //now read lens id
                if(*sensor_id == GC5025A_SENSOR_ID)
                {
                    if(gc5025a_otp_read != 1) 
                    {
                        LOG_INF("GC5025ABLX3P:get_imgsensor_id() first time read otp!\n");
                        gc5025a_gcore_identify_otp();
                    }
                    else
                    {
                         LOG_INF("GC5025ABLX3P:get_imgsensor_id() otp already read!\n");
                    }
                    if(gc5025a_otp_info.infowbvalid && (gc5025a_otp_info.lens_id == 0x02) && (gc5025a_otp_info.module_id == 0x49) )
                    {
                        LOG_INF("GC5025ABLX3P:get_imgsensor_id() Module BOLIXIN3P lensid=0x%x\n", gc5025a_otp_info.lens_id);
                    }
                    else if(gc5025a_otp_info.infowbvalid && (gc5025a_otp_info.lens_id == 0x02)&& (gc5025a_otp_info.module_id == 0x56) )
                    {
                        LOG_INF("GC5025TSP4P:get_imgsensor_id() Module TSP4P lensid=0x%x\n", gc5025a_otp_info.lens_id);
                        *sensor_id+=1;
                    }
                    else
                    {
                        LOG_INF("GC5025ABLX3P:get_imgsensor_id() Not a  Bolixin Module!\n");
                    }
                }
                if (*sensor_id == imgsensor_info.sensor_id) {

                	LOG_INF("GC5025ABLX3P:get_imgsensor_id() i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                	return ERROR_NONE;
            	}
            	LOG_INF("GC5025ABLX3P:get_imgsensor_id() Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            	retry--;
        	} while(retry > 0);
        	i++;
        	retry = 2;
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
			...
			if(sensor_id == GC5025A_SENSOR_ID)
            {
                    if(gc5025a_otp_read != 1) 
                    {
                        LOG_INF("GC5025ABLX3P:open() first time read otp!\n");
                        gc5025a_gcore_identify_otp();
                    }
                    else
                    {
                         LOG_INF("GC5025ABLX3P:open() otp already read!\n");
                    }
                    if(gc5025a_otp_info.infowbvalid && (gc5025a_otp_info.lens_id == 0x02)&& (gc5025a_otp_info.module_id == 0x49) )
                    {
                        LOG_INF("GC5025ABLX3P:open() Module BOLIXIN3P lensid=0x%x\n", gc5025a_otp_info.lens_id);
                    }
                    else if(gc5025a_otp_info.infowbvalid && (gc5025a_otp_info.lens_id == 0x02) && (gc5025a_otp_info.module_id == 0x56) )
                    {
                        LOG_INF("GC5025TSP4P:open() Module TSP4P lensid=0x%x\n", gc5025a_otp_info.lens_id);
                        sensor_id+=1;
                    }
                    else
                    {
                        LOG_INF("GC5025ABLX3P:open() Not a  Bolixin Module!\n");
                    }
            }

            if (sensor_id == imgsensor_info.sensor_id) {
                LOG_INF("GC5025ABLX3P:open() i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
                break;
            }
            LOG_INF("GC5025ABLX3P:open() Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        if (sensor_id == imgsensor_info.sensor_id)
...
UINT32 GC5025A_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
