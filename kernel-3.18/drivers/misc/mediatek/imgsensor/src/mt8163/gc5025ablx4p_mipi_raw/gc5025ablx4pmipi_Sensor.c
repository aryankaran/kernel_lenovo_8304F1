...
#include "gc5025ablx4pmipi_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "GC5025_camera_sensor"
#define LOG_1 LOG_INF("GC5025 TSP4P MODULE,MIPI 2LANE\n")
/****************************   Modify end    *******************************************/
...
static imgsensor_info_struct imgsensor_info = {
       .sensor_id = GC5025ABLX4P_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
...
#define INFO_ROM_START		0x01    //info start
#define INFO_WIDTH       	0x08
#define WB_ROM_START      	0x11    //wb  start
#define WB_WIDTH          	0x05
#define GOLDEN_ROM_START  	0x1c    //af start
...
static kal_uint8 gc5025_otp_read;
...
#ifdef GC5025OTP_FOR_CUSTOMER	
	gc5025_select_page_otp(otp_page1);
	infowb_flag = gc5025_read_otp(0x00);//read InfoWBFlag
	af_flag =     gc5025_read_otp(0x1d);//read AFFlag
	LOG_INF("GC5025_OTP : infowb_flag = 0x%x , af_flag = 0x%x\n",infowb_flag,af_flag);
//INFO&WB
        if(infowb_flag == 0x01) 
        {   
            gc5025_otp_info.infowbvalid = 1;
            gc5025_read_otp_group(1, &info[0], 14);       
        }
        else if(infowb_flag == 0x07)
        {
            gc5025_otp_info.infowbvalid = 2;
            gc5025_read_otp_group(15, &info[0], 14);
        }
        else
        {   
            gc5025_otp_info.infowbvalid = 0;
            LOG_INF("infowb_flag  invalid !!\n");
        }
        if (gc5025_otp_info.infowbvalid) 
        {   
            check = 0;
            for (i=0;i<13;i++)
            {
            ...
            if ((check % 255 + 1) == info[13])
            {
                ...
                gc5025_otp_info.rg_gain = info[5]<<8|info[6];
                gc5025_otp_info.bg_gain = info[7]<<8|info[8];
                gc5025_otp_info.golden_rg = info[9]<<8|info[10];
                gc5025_otp_info.golden_bg = info[11]<<8|info[12];
            }
            else
            {   gc5025_otp_info.infowbvalid = 0;
        ...
        if(af_flag == 0x01) 
        {   
            gc5025_otp_info.afvalid = 1;
            gc5025_read_otp_group(30, &afcode[0], 4);       
        }
        else if(af_flag == 0x07)
        {   
            gc5025_otp_info.afvalid = 2;
            gc5025_read_otp_group(34, &afcode[0], 4);
        }
        else
        {   
            gc5025_otp_info.afvalid = 0;
            LOG_INF("af_flag  invalid !!\n");
        }
        if (gc5025_otp_info.afvalid) 
        {
            check = 0;
            for (i=0;i<3;i++)
            {
            ...
            {   
                ...
                gc5025_otp_info.af_inf =   InfHigh<<8|afcode[1];
                gc5025_otp_info.af_macro = MacHigh<<8|afcode[2];
            }
            else
            {
                gc5025_otp_info.afvalid=0;
    ...
    LOG_INF("GC5025_OTP_GOLDEN:af_inf=0x%x\n",gc5025_otp_info.af_inf);
	LOG_INF("GC5025_OTP_GOLDEN:af_macro=0x%x\n",gc5025_otp_info.af_macro);
	if(!infowb_flag || !af_flag ||(gc5025_otp_info.af_inf < 100))
	{
		LOG_INF("gc5025_otp_info.OTP ERROR! Set preview color to green......");
		gc5025_otp_info.infowbvalid =0xff;
		gc5025_otp_info.rg_gain=10;
		gc5025_otp_info.bg_gain=1999;
	}
...
{	
    kal_uint8 flag_doublereset = 0,flag_GC5025 = 0;
...
int GetAfCode_GC5025Ablx4p(u16 *Inf,u16 *Macro)
{
    if (gc5025_otp_info.afvalid)
    {
        *Macro = gc5025_otp_info.af_macro;
		*Inf   = gc5025_otp_info.af_inf;
    }
    else
    {
        *Macro  = 400;
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    kal_uint8 i = 0;
    kal_uint8 retry = 2;
    LOG_INF("GC5025TSP4P:get_imgsensor_id +++++\n");
    	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        	...
        	do {
            	*sensor_id = return_sensor_id();
                ...
                {
                    if(gc5025_otp_read != 1) 
                    {
                        LOG_INF("GC5025TSP4P:get_imgsensor_id() first time read otp!\n");
                        gc5025_gcore_identify_otp();
                    }
                    else
                    {
                         LOG_INF("GC5025TSP4P:get_imgsensor_id() otp already read!\n");
                    }
                    if(gc5025_otp_info.infowbvalid && (gc5025_otp_info.lens_id == 0x02) && (gc5025_otp_info.module_id == 0x56) )
                    {
                        LOG_INF("GC5025TSP4P:get_imgsensor_id() Module  lensid=0x%x\n", gc5025_otp_info.lens_id);
						*sensor_id+=1;
                    }

                    else
                    {	
                        LOG_INF("GC5025TSP4P:get_imgsensor_id() Not a  TSP Module!\n");
                    }
                }
            	if (*sensor_id == imgsensor_info.sensor_id) {
                	LOG_INF("GC5025TSP4P:get_imgsensor_id() stoneadd  success i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                	return ERROR_NONE;
            	}
            	LOG_INF("GC5025TSP4P:get_imgsensor_id() Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            	retry--;
        	} while(retry > 0);
        	i++;
        	retry = 2;
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
			...
			if(sensor_id == GC5025A_SENSOR_ID)
            {
                    if(gc5025_otp_read != 1) 
                    {
                        LOG_INF("GC5025TSP4P:open() first time read otp!\n");
                        gc5025_gcore_identify_otp();
                    }
                    else
                    {
                         LOG_INF("GC5025TSP4P:open() otp already read!\n");
                    }
                    if(gc5025_otp_info.infowbvalid && (gc5025_otp_info.lens_id == 0x02) && (gc5025_otp_info.module_id == 0x56))
                    {
                        LOG_INF("GC5025TSP4P:open() Module  lensid=0x%x\n", gc5025_otp_info.lens_id);
						sensor_id+=1;
                    }

                    else
                    {
                        LOG_INF("GC5025TSP4P:open() Not a  TSP Module!\n");
                    }
            }

            if (sensor_id == imgsensor_info.sensor_id) {
                LOG_INF("GC5025TSP4P:open() i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
                break;
            }
            LOG_INF("GC5025TSP4P:open() Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        if (sensor_id == imgsensor_info.sensor_id)
        {
            LOG_INF("GC5025TSP4P:open() Read sensor id success!, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
            break;
        }
        retry = 2;
    }
...
UINT32 GC5025ABLX4P_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
    /* To Do : Check Sensor status here */
    if (pfFunc!=NULL)
        *pfFunc=&sensor_func;
    return ERROR_NONE;
}    /*    GC5025ABLX4PMIPI_RAW_SensorInit    */
