...
/*Begin:add by zch, for Camera Hardwareinfo*/
...
/*End:add by zch, for Camera Hardwareinfo*/
...
static inline int adopt_CAMERA_HW_Open(void)
{
	...
	KD_IMGSENSOR_PROFILE_INIT();
	/* power on sensor */
	/*Begin:Modify by zch, for Camera Hardwareinfo */
			...
			else
			{
				...
				for(i = 0;i < KDIMGSENSOR_MAX_INVOKE_DRIVERS;i++)
				{
					if(g_bEnableDriver[i])
					{
						...
						{
							if(strcmp(g_invokeSensorNameStr[i],"gc5025amipiraw")==0)
								memcpy(mainCameraName,"gc5025amipiraw-5M",CAM_NAME_LEN);
							else if(strcmp(g_invokeSensorNameStr[i],"hi556mipiraw")==0)
								memcpy(mainCameraName,"hi556mipiraw-5M",CAM_NAME_LEN);
							else if(strcmp(g_invokeSensorNameStr[i],"hi556qun4pmipiraw")==0)
								memcpy(mainCameraName,"hi556qun4pmipiraw-5M",CAM_NAME_LEN);
							else
						...
						{
							if(strcmp(g_invokeSensorNameStr[i],"sp2509mipiraw")==0)
								memcpy(subCameraName,"sp2509mipiraw-2M",CAM_NAME_LEN);
							else if(strcmp(g_invokeSensorNameStr[i],"gc2375mipiraw")==0)
								memcpy(subCameraName,"gc2375mipiraw-2M",CAM_NAME_LEN);
							else
	...
	/*End:Modify by zch, for Camera Hardwareinfo*/
