...
/*Begin:chenguanghe add for Runin control precentage,20170412 */
int g_runin_battery_test_flag = 0; 
kal_bool g_charger_should_not_open = KAL_FALSE;

static int g_chr_type_second_check = 0;
/*End:chenguanghe add for Runin control precentage,20170412 */

static int cmd_otg_enable = -1;
struct wake_lock battery_suspend_lock;
struct wake_lock battery_fg_lock;
struct wake_lock draw_lock;
...
void check_ACA_power(void)
{
	...
	if((0x0 == hw_otg_get_aca_type())) {
		cmd_otg_enable = KAL_TRUE;
		battery_charging_control(CHARGING_CMD_ENABLE_OTG, &cmd_otg_enable);
	}
}
...
/* ///////////////////////////////////////////////////////////////////////////////////////// */
static ssize_t show_Ycable_charging_enable(struct device *dev, struct device_attribute *attr, char *buf)
{
...
static ssize_t store_Ycable_charging_enable(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
...
void mt_battery_GetBatteryData(void)
{
	...
	battery_log(BAT_LOG_CRTI,
	"AvgVbat=(%d,%d),AvgI=(%d,%d),Charger(%d,%d),AvgT=(%d,%d),SOC=(%d,%d),UI_SOC=%d,ZCV=%d bcct:%d:%d I:%d Ibat:%d,Nbat:%s,Idvol=%d\n",
		    BMT_status.bat_vol, bat_vol, BMT_status.ICharging, ICharging,
		    BMT_status.charger_type,BMT_status.charger_vol, BMT_status.temperature, temperature,
		previous_SOC, BMT_status.SOC, BMT_status.UI_SOC,BMT_status.ZCV,
		g_bcct_flag, get_usb_current_unlimited(), get_bat_charging_current_level(),BMT_status.IBattery,g_battery_manufacturer,g_fg_battery_id);
...
// zch new for  Charger over voltage cannot recharger problem FAQ14809
...
#else
	{
		/*Begin:chenguanghe add for MMI Current Test delay 10s ,20170418 */
		usb_update(&usb_main);
		ac_update(&ac_main);
		wireless_update(&wireless_main);
		battery_update(&battery_main);
		/*End:chenguanghe add for MMI Current Test delay 10s ,20170418 */
...
*/
	...
	if (upmu_is_chr_det() == KAL_TRUE) {
		...
		/*Begin:chenguanghe add for Runin control precentage,20170412 */
			if((NONSTANDARD_CHARGER == BMT_status.charger_type)&& (0 == g_chr_type_second_check))
			{
				mt_charger_type_detection();
				battery_log(BAT_LOG_CRTI, "[BAT_thread]CHR_Type_num=%d,second_check=%d\r\n", BMT_status.charger_type,g_chr_type_second_check);
			}
		/*End:chenguanghe add for Runin control precentage,20170412 */
...
*/
		...
		g_chr_type_second_check = 0;//chenguanghe add for Runin control precentage,20170412
...
static long adc_cali_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	//unsigned int icharging_enable = KAL_FALSE;//chenguanghe add for Runin control precentage,20170412
...
/*Begin:chenguanghe add for Runin control precentage,20170412 */
	case BAT_THREAD_CTRL: /* for runtime test */
		...
		if (*user_data_addr == 2)
		{
			...
			battery_log(BAT_LOG_CRTI,"runin charging test close charger\n");
		}
		else if (*user_data_addr == 1 || *user_data_addr == 3) 
		{
			//icharging_enable = KAL_TRUE;
			g_charger_should_not_open = KAL_FALSE;                  /* open charger */
			battery_log(BAT_LOG_CRTI,"runin charging test open charger\n");
		}
...
static int battery_probe(struct platform_device *dev)
{
	...
	if (g_platform_boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT || g_platform_boot_mode == LOW_POWER_OFF_CHARGING_BOOT) {
		wake_lock_init(&draw_lock, WAKE_LOCK_SUSPEND, "charger wakelock");
		wake_lock(&draw_lock);
	}
#endif
