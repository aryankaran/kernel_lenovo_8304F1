...
char *g_battery_manufacturer = NULL;
...
extern int IMM_GetOneChannelValue_Cali(int Channel, int *voltage);
static enum hq_batteryID hq_get_battery_id(void)
{
	int ret = 0;
	int id_volt = 0;
	enum hq_batteryID id;

	ret = IMM_GetOneChannelValue_Cali(12, &id_volt);
	if (ret != 0)
	...
	if (id_volt < 1000000) {
		id = HQ_BATTERY_XWD;
		g_battery_manufacturer = "XWD-ATL";
	} else {
		id = HQ_BATTERY_FMT;
		g_battery_manufacturer = "FMT-CLT";
	}
