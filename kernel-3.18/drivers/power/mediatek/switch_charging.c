...
CHR_CURRENT_ENUM g_stander_charging_temp_CC_value = CHARGE_CURRENT_0_00_MA;
unsigned int g_usb_state = USB_UNCONFIGURED;
extern int g_runin_battery_test_flag;//chenguanghe add for Runin control precentage,20170412
extern kal_bool g_charger_should_not_open;
...
static BATTERY_VOLTAGE_ENUM select_jeita_cv(void)
{
	...
	if(!g_runin_battery_test_flag) {
		...
		} else if (g_temp_status == TEMP_POS_10_TO_POS_45)
		{
			...
			g_stander_charging_temp_CC_value = JEITA_TEMP_POS_10_TO_POS_45_CV_CURRENT;
		} else if (g_temp_status == TEMP_POS_0_TO_POS_10)
		{
			cv_voltage = JEITA_TEMP_POS_0_TO_POS_10_CV_VOLTAGE;
			g_stander_charging_temp_CC_value = JEITA_TEMP_POS_0_TO_POS_10_CV_CURRENT;
		} else if (g_temp_status == TEMP_NEG_10_TO_POS_0)
		{
			cv_voltage = JEITA_TEMP_NEG_10_TO_POS_0_CV_VOLTAGE;
			g_stander_charging_temp_CC_value = JEITA_TEMP_NEG_10_TO_POS_0_CV_CURRENT;
		} else if (g_temp_status == TEMP_BELOW_NEG_10)
		{
			cv_voltage = JEITA_TEMP_BELOW_NEG_10_CV_VOLTAGE;
			g_stander_charging_temp_CC_value = JEITA_TEMP_BELOW_NEG_10_CV_CURRENT;
		} else
		{
			cv_voltage = BATTERY_VOLT_04_200000_V;
			g_stander_charging_temp_CC_value = CHARGE_CURRENT_500_00_MA;
		}
	}else {
		cv_voltage = JEITA_TEMP_POS_10_TO_POS_45_CV_VOLTAGE;
		g_stander_charging_temp_CC_value = JEITA_TEMP_POS_10_TO_POS_45_CV_CURRENT;
	}
	/*add by zhengxianjin,start set cc 20170426*/
	if (BMT_status.charger_type == STANDARD_CHARGER) {
		g_temp_CC_value = g_stander_charging_temp_CC_value;
		battery_log(BAT_LOG_CRTI, "%s: CC_value = %dmA\n", __func__, g_temp_CC_value);
		battery_charging_control(CHARGING_CMD_SET_CURRENT, &g_temp_CC_value);
	}
	/*add by zhengxianjin,end set cc 20170426*/
...
unsigned int set_bat_charging_current_limit(int current_limit)
{
	...
	if ((current_limit != -1) && (!g_runin_battery_test_flag)) {
		g_bcct_flag = 1;
		switch (BMT_status.charger_type) {
		case STANDARD_HOST:
...
static unsigned int charging_full_check(void)
{
	...
	if (status == KAL_TRUE) {
		printk("[%s] status = %d, BMT_status.bat_vol = %d",__func__,status, BMT_status.bat_vol);
		printk("[%s] BMT_status.temperature = %d, g_full_check_count= %d!\n", __func__,BMT_status.temperature, g_full_check_count);
		g_full_check_count++;
...
//mtk modify beging 20170517
		if (BMT_status.charger_type == STANDARD_CHARGER)
			g_temp_input_CC_value = batt_cust_data.ac_charger_current;
		else
			g_temp_input_CC_value = batt_cust_data.charging_host_charger_current;
//mtk modify end 20170517
...
static void mtk_select_ichg_aicr(void)
{
	...
	if (g_charger_should_not_open == KAL_TRUE)
		enable_charger = KAL_FALSE;
...
static void pchr_turn_on_charging(void)
{
	...
	if (KAL_FALSE == BMT_status.bat_in_charging_enable)
		charging_enable = KAL_FALSE;
...
#ifndef CONFIG_MTK_SM5414_SUPPORT
	if (BMT_status.bat_charging_state == CHR_ERROR)
#else
	/* When bat_charging_state is CHR_BATFULL, turn off the charging battery function */
...
PMU_STATUS BAT_ConstantCurrentModeAction(void)
{
	...
	if (charging_full_check() == KAL_TRUE) {
                printk("zch charge---in BAT_ConstantCurrentModeAction  bat_full = KAL_TRUE");
		BMT_status.bat_charging_state = CHR_BATFULL;
		BMT_status.bat_full = KAL_TRUE;
		g_charging_full_reset_bat_meter = KAL_TRUE;
	}
