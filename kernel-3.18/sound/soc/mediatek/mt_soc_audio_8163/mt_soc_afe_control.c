...
void SetIrqStatusClear(uint32 Irqmode)
{
	if (checkDllinkMEMIfStatus() == false) {
		pr_warn("SetIrqStatusClear Irq 0x%x", Irqmode);
		Afe_Set_Reg(AFE_IRQ_MCU_CLR, 1, 0xff);
	}
}
...
bool SetIrqMcuCounter(uint32 Irqmode, uint32 Counter)
{
	...
	PRINTK_AUDDRV("%s Irqmode %d, Counter %d, mIrqMcuCounter %d, pid %d, tid %d\n",
		__func__, Irqmode, Counter, mAudioMcuMode[Irqmode]->mIrqMcuCounter,
	...
	if (dumplog) {
		...
		if (localctl.u4UnderflowCnt) {
			for (index = 0; index < localctl.u4UnderflowCnt && index < DL_ABNORMAL_CONTROL_MAX; index++) {
				pr_warn("underflow [%d/%d] MemType %d\n", index,
				 localctl.u4UnderflowCnt, localctl.MemIfNum[index]);
			}
		}
	}
}
