...
static int mtk_pcm_dl2_stop(struct snd_pcm_substream *substream)
{
	...
	SetIrqStatusClear(Soc_Aud_IRQ_MCU_MODE_IRQ1_MCU_MODE);
			...
			void __user *dst, snd_pcm_uframes_t count)
{
	...
	if (!access_ok(VERIFY_READ, data_w_ptr, count)) {
		pr_err("AudDrv_write Invalid ptr %p, count %lu", data_w_ptr, count);
		return 0;
	}
	...
	if (copy_size != 0) {
		...
		if (Afe_WriteIdx_tmp + copy_size < Afe_Block->u4BufferSize) {	/* copy once */
			if (!access_ok(VERIFY_READ, data_w_ptr, copy_size)) {
				pr_err("AudDrv_write 0ptr invalid data_w_ptr=%p, size=%d",
